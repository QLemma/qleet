...
def test_analyzer_list():
    ...
    circuit = qleet.interface.circuit.CircuitDescriptor(qaoa.qaoa_circuit, qaoa.params, qaoa.qaoa_cost)
    solver = qleet.simulators.pqc_trainer.PQCSimulatedTrainer(circuit)
    metric = qleet.examples.qaoa_maxcut.MaxCutMetric(graph)

    logger_1 = qleet.analyzers.loss_landscape.LossLandscapePlotter(solver, metric, dim=2)
